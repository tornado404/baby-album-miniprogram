...
Page({
    ...
    uploadAvatar: function (filePath) {
        if (!filePath || !this.data.babyId)
            return;
        var token = tokenManager.getAccessToken();
        var _this = this;
        wx.uploadFile({
            url: api_1.API_CONFIG.baseURL + '/babies/' + this.data.babyId + '/avatar',
            filePath: filePath,
            name: 'file',
            header: { 'Authorization': 'Bearer ' + token },
            success: function (res) {
    ...
    updateBaby: function (babyData) {
        ...
        var token = tokenManager.getAccessToken();
        wx.request({
            url: api_1.API_CONFIG.baseURL + '/babies/' + this.data.babyId,
            method: 'PUT',
            data: babyData,
        ...
        wx.request({
            ...
            success: function (res) {
                ...
                if (res.statusCode === 201 && res.data && res.data.id) {
                    try {
                        wx.setStorageSync(storage_keys_1.STORAGE_KEYS.currentBabyId, res.data.id);
                    }
                    catch (e) { }
