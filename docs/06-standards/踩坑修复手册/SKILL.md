...
- [A. 运行时错误](#a-运行时错误)
  ...
  - [A5. MinIO 上传 403 SignatureDoesNotMatch](#a5-minio-上传-403-signaturedoesnotmatch)
...
### A5. MinIO 上传 403 SignatureDoesNotMatch

**症状**

小程序上传图片到 MinIO，PUT 请求返回 403：
...
上传 URL 中带有 `?mock=upload&key=...` 参数。

**根因**

三个问题叠加导致：

1. **`MINIO_ENDPOINT` 配置错误**：`.env` 中 `MINIO_ENDPOINT=minio:9000`，但 Docker 网络中 MinIO 容器名是 `baby-minio`。`check_minio_available()` 检测 `http://minio:9000/minio/health/live` → DNS 解析失败 → 缓存 `False`。同时 MinIO 容器未加入 `server_baby-network` 网络（在 `server_default` 中），导致即使修正主机名也无法连通。

2. **Dev 模式降级返回 mock URL**：`DEBUG=true` + `check_minio_available()=False` → `generate_presigned_upload_url()` 进入降级分支，返回 `?mock=upload` 的 mock URL。客户端 PUT 到 mock URL → MinIO 返回 403 AccessDenied（URL 无签名）。

3. **AWS Sig V4 签名 payload hash 大小写错误**：修正网络问题后，`_sign_presigned_url` 使用 `unsigned-payload`（小写），但 AWS/MinIO 规范要求 `UNSIGNED-PAYLOAD`（大写）。`_sign_request_headers` 使用 `sha256("")` 作为 payload hash，但实际发送了非空内容，导致 `XAmzContentSHA256Mismatch`。

**修复**

1. 修正 `.env` 中的 MinIO 主机名：

```diff
- MINIO_ENDPOINT=minio:9000
+ MINIO_ENDPOINT=baby-minio:9000
```

2. 将 MinIO 容器加入 `server_baby-network`：

```bash
docker network connect server_baby-network baby-minio
```

3. 修正签名 payload hash 大小写（`server/app/services/file_service.py`）：

```diff
  # _sign_presigned_url 中：
- f"unsigned-payload"
+ f"UNSIGNED-PAYLOAD"

  # _sign_request_headers 中：
- payload_hash = hashlib.sha256(b"").hexdigest()
+ payload_hash = "UNSIGNED-PAYLOAD"
```

4. 修改 `.env` 后需重建容器（仅 `docker restart` 不会重新加载 `.env`）：

```bash
docker rm -f baby-api
docker run -d --name baby-api --env-file .env \
  -p 8000:8000 --network server_baby-network \
  -v /home/linaro/baby-album/server/app:/app/app \
  --restart unless-stopped baby-api-base:latest
```
...
| 方案 | 做法 | 收益 |
|------|------|------|
| **A. 容器名与 .env 统一** | docker-compose.yml 中容器名固定为 `baby-minio`，`.env` 中 `MINIO_ENDPOINT=baby-minio:9000` | 根治主机名不匹配 |
| **B. docker-compose 一体启动** | MinIO 纳入 `docker-compose.arm.yml`，确保所有服务在同一网络 | 避免网络隔离 |
| **C. 签名用 minio SDK** | 用 `minio.Minio.presigned_put_object()` 替代手写 Sig V4 | SDK 跟着规范走，大小写不会错 |
| **D. 启动时校验 MinIO 连通** | `lifespan` 中 `check_minio_available()`，不可达就 WARN 或拒绝启动 | 早发现配置错误 |

建议至少做 **A + B**，长期考虑 **C**（移除手写签名逻辑）。
